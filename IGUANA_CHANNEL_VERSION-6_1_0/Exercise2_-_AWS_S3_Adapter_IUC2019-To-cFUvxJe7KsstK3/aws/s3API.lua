...
local function DeleteS3(canonicalendpoint)
   local mydeleteheaders = headerUtil.deleteAwsViaAuthorizationHeader(canonicalendpoint)
   local url = awsConfigs.ENDPOINT..canonicalendpoint
   local r, e, h = net.http.delete{url = url, headers = mydeleteheaders, method='DELETE', body = content, live = awsConfigs.DELETELIVE, timeout= awsConfigs.POSTTIMEOUT}
   return r,e, h
end
-- Public functions
...
function s3API.uploadFile(content, canonicalendpoint)
   ...
   if tonumber(e) == 200 then
      iguana.logInfo('New File is uploaded')
      return true
   else
      iguana.logInfo('File upload Failed')
      return false
   end
end

function s3API.deleteFile(canonicalendpoint)
    local r, e, h, re = retry.call{func=DeleteS3, arg1=canonicalendpoint, retry=awsConfigs.RETRY, pause=awsConfigs.RETRYPAUSE, funcname='DeleteS3', errorfunc=WebServiceError}
     if tonumber(e) == 204 then
      iguana.logInfo('File is deleted successfully')
      return true
   else
      iguana.logInfo('File delete Failed')
      return false
   end
end
...
"ParameterTable": false,
"Usage": "s3API.uploadFile(content, canonicalendpoint)",
"Examples": [
"-- Define S3 bucket absolute file path
 local canonicalendpoint = '/JSON/patient-schema.json'<br />
 -- Put data in S3
...
local DeleteHelp=[[{
"Returns": [{"Desc": "The file contents from the configured AWS S3 bucket."}],
"Title": "s3API.deleteFile",
"Parameters": [{ "canonicalendpoint": {"Desc": "S3 bucket absolute file path."}}],
"ParameterTable": false,
"Usage": "s3API.deleteFile(canonicalendpoint)",
"Examples": [
"-- Define S3 bucket absolute file path
 local canonicalendpoint = '/JSON/patient-schema.json'<br />
 -- Put data in S3
 s3API.deleteFile(content, canonicalendpoint)<br/>
 -- Get data in S3
 local file = s3API.readFile(canonicalendpoint)<br/>
-- Delete file in S3
 s3API.deleteFile(canonicalendpoint)"
],
"Desc": "This function deletes a file from the configured AWS S3 bucket."
}]]

help.set{input_function=s3API.deleteFile, help_data=json.parse{data=DeleteHelp}}
return s3API